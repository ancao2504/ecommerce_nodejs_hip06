...
const getCurrent = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json({
        ...
        results: user ? user : 'User not found'
    })
})
...
const forgotPassword = asyncHandler(async (req, res) => {
    ...
    const results = await sendMail(data)
    return res.status(200).json({
        success: true,
        results
    })
...
const updateUser = asyncHandler(async (req, res) => {
    ...
    const response = await userModel.findByIdAndUpdate(_id, req.body, { new: true }).select('-password -role -refreshToken')
    return res.status(200).json({
        success: response ? true : false,
